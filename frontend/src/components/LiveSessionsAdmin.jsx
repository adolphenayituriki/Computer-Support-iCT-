...
const STATUSES = { scheduled: 'bg-cshub-blue/10 text-cshub-blue', live: 'bg-emerald-100 text-emerald-700', ended: 'bg-slate-100 text-slate-500', cancelled: 'bg-red-100 text-red-500' };

export default function LiveSessionsAdmin() {
  const { showToast } = useToast();
  const [sessions, setSessions] = useState([]);
  const [monthFilter, setMonthFilter] = useState('all');
  const filtered = monthFilter === 'all' ? sessions : sessions.filter((s) => monthLabel(s.scheduledAt) === monthFilter);
      ...
      <div className="fixed inset-0 z-[999] bg-slate-950">
        <div className="absolute top-0 left-0 right-0 z-10 flex items-center justify-between bg-slate-900/95 px-4 py-2 backdrop-blur-sm border-b border-slate-800">
          <div className="flex items-center gap-3">
            <FaVideo className="h-4 w-4 text-slate-500" />
            <div>
  ...
  return (
    <div className="space-y-3">
          ...
          <div className="w-full max-w-md rounded-xl border border-slate-200 bg-white shadow-2xl animate-in fade-in zoom-in-95" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="px-5 py-4 space-y-3">
              ...
              <div ref={userDropdownRef} className="relative">
                ...
                {form.hostId && <p className="mt-1 text-[10px] text-slate-500 font-medium">✓ Host selected: {userSearch}</p>}
              </div>
              ...
              <button type="submit" className="w-full rounded-lg bg-cshub-blue py-2 text-xs font-semibold text-white hover:bg-[#3f7ee8] disabled:opacity-50" disabled={submitting}>
      ...
      <div className="flex items-center justify-between">
        <h3 className="flex items-center gap-2 text-sm font-semibold text-slate-900"><FaVideo className="h-3.5 w-3.5 text-slate-400" /> Live Sessions ({sessions.length})</h3>
        <button onClick={() => { setForm(FETCH_FORM); setUserSearch(''); setEditSession(null); setShowCreate(true); }} className="flex items-center gap-1.5 rounded-lg bg-cshub-blue px-3 py-1.5 text-[11px] font-semibold text-white hover:bg-[#3f7ee8]"><FaPlus /> New Session</button>
      </div>

      {sessions.length > 0 && <MonthFilter items={sessions} value={monthFilter} onChange={setMonthFilter} getDate={(s) => s.scheduledAt} />}
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-slate-200 bg-white py-8 text-center">
          <p className="text-xs text-slate-400">No sessions in the selected month.</p>
        </div>
      ) : (
        <div className="space-y-6">
          {groupByMonth(filtered, (s) => s.scheduledAt).map((g) => (
            <div key={g.label} className="space-y-2">
              <MonthHeader label={g.label} count={g.list.length} />
              <div className="grid grid-cols-1 gap-2 lg:grid-cols-2">
                {g.list.map((s) => (
                  <div key={s._id} className="rounded-xl border border-slate-200 bg-white p-3">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex items-center gap-1 shrink-0 flex-wrap justify-end">
                  {s.status === 'live' && (
                    <button onClick={() => copyLink(s)} className={cn("flex items-center gap-1 rounded-lg px-2.5 py-1 text-[10px] font-semibold transition-colors", copied === s._id ? "bg-emerald-100 text-emerald-600" : "bg-cshub-blue/10 text-cshub-blue hover:bg-cshub-blue/20")}>
                  ...
                  {s.status === 'live' && (
                    <button onClick={() => joinAsHost(s)} className="flex items-center gap-1 rounded-lg bg-cshub-blue px-2.5 py-1 text-[10px] font-semibold text-white hover:bg-[#3f7ee8] animate-pulse"><FaEye className="text-[8px]" /> Join</button>
                  )}
                  {s.status === 'live' && (
